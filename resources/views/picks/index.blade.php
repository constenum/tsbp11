                    ...
                    <div class="p-2 md:p-4 lg:p-6 text-gray-900">
                        ...
                        <form method="POST" action="{{ route('picks.store') }}">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <div class="flex items-center px-4 lg:px-6">
                                                    <input id="team1" name="game{{ $game->id }}" type="radio" value="{{ $game->away_team_id }}"
                                                           ...
                                                           @readonly($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and in_array($game->away_team_id, $picks))
                                                           @disabled($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and !in_array($game->away_team_id, $picks))
                                                           @disabled($reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and !in_array($game->away_team_id, $picks))
                                                           />
                                                    <label for="team1"
                                                           class="ml-3 block leading-6 @if($game->away_spread < 0) uppercase @endif text-gray-900 @if($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) or $reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5)) text-gray-400 @endif">{{ $game->away_team->yahoo_name }}
                                                ...
                                                <div class="flex items-center px-4 lg:px-6">
                                                    <input id="team2" name="game{{ $game->id }}" type="radio" value="{{ $game->home_team_id }}"
                                                           ...
                                                           @readonly($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and in_array($game->home_team_id, $picks))
                                                           @disabled($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and !in_array($game->home_team_id, $picks))
                                                           @disabled($reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) and !in_array($game->home_team_id, $picks))
                                                           />
                                                    <label for="team2"
                                                           class="ml-3 block leading-6 @if($game->home_spread < 0) uppercase @endif text-gray-900 @if($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) or $reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5)) text-gray-400 @endif">{{ $game->home_team->yahoo_name }}
                                                ...
                                                <div class="flex justify-end mt-2 px-2 md:px-4">
                                                    <button type="button" id="radioReset"
                                                            ...
                                                            @if($game->start_at < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5) or $reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5)) disabled @endif>
                                                        {{ __('Clear Pick') }}
                                                    </button>
                                                </div>
                                            </div>
                            ...
                            <div class="flex justify-end px-2 md:px-4 py-3 font-normal">
                                <div
                                    class="font-semibold  @if($reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5)) hidden @endif">
                            ...
                            <div
                                class="flex justify-end bg-white px-2 md:px-4 py-3  @if($reveal_picks < \Carbon\Carbon::now()->setTimezone("America/New_York")->subHours(5)) hidden @endif">
                                <x-danger-button>
                                    {{ __('Reset') }}
                                </x-danger-button>
                                <x-button class="ml-4">
